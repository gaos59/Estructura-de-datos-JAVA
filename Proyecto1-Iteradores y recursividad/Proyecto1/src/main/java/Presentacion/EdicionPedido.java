...
public class EdicionPedido extends javax.swing.JFrame {
    ...
    private Pedido nuevoEditado;
    private MostrarEditarPedidos mostrarEditarPedidos;
    
    public EdicionPedido(MostrarEditarPedidos mostrarEditarPedidos,Pedido pedido) {
        
        if(pedido == null){
            return;
        }
        this.mostrarEditarPedidos = mostrarEditarPedidos;
        initComponents();
        
        //Colocar valores actuales del pedido.
        try {
            this.nuevoEditado = pedido;
            
            LblIDMostrar.setText(String.valueOf(pedido.getId()));
            CbxTipoCamiseta.setSelectedItem(pedido.getTipoCamiseta());
            CbxTalla.setSelectedItem(pedido.getTalla());
            TxfCantidad.setText(String.valueOf(pedido.getCantidad()));
            TxfCodigoDiseno.setText(pedido.getCodigoDiseno());
            CbxTipoPedido.setSelectedItem(pedido.getTipoPedido());
            TxfDireccionEntrega.setText(pedido.getDireccionEntrega());
            CbxFormaPago.setSelectedItem(pedido.getFormaPago());
            TxfTelefonoCliente.setText(pedido.getTelefonoCliente());
            TxfNombreCliente.setText(pedido.getNombreCliente());
            if ("Envío a domicilio".equals(String.valueOf(CbxTipoPedido.getSelectedItem()))) {
                TxfDireccionEntrega.setEnabled(true);
            }
        }
        catch (Exception e){
            JOptionPane.showMessageDialog(null, "No se puede colocar valores del pedido", "Error", JOptionPane.ERROR_MESSAGE);
        }
        ...
        try {
            ...
            addWindowListener(new WindowAdapter() {
                ...
                public void windowClosed(WindowEvent e) {
                   mostrarEditarPedidos.setEditarBtnEnabled(true);
                }
            });
        }
        catch (Exception e){
            JOptionPane.showMessageDialog(null, "No se puede habilitar el boton para editar pedidos", "Error", JOptionPane.ERROR_MESSAGE);
        }
    ...
    private void BtnAgregarPedidoActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_BtnAgregarPedidoActionPerformed
        ...
        try {
            int id = Integer.parseInt(LblIDMostrar.getText()); // Asignar un ID secuencial;
            ...
            nuevoEditado = new Pedido( id, tipoCamiseta, talla, cantidad, codigoDiseno, tipoPedido, direccionEntrega, formaPago, telefonoCliente, nombreCliente);
        
            mostrarEditarPedidos.EditarPedido(nuevoEditado);
            
            
            JOptionPane.showMessageDialog(null, "Pedido modificado ID: " + nuevoEditado.getId() + "\n" + nuevoEditado.getTipoCamiseta(), "Información", JOptionPane.INFORMATION_MESSAGE);
            this.dispose();
        }
